...
const useAnimationLoop = (
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      const deltaTime = Math_max(0, timestamp - lastTimestampRef.current) / 1000;
      lastTimestampRef.current = timestamp;

      const target = isHovered && hoverSpeed !== undefined ? hoverSpeed : targetVelocity;

      const easingFactor = 1 - Math_exp(-deltaTime / ANIMATION_CONFIG.SMOOTH_TAU);
      currentVelocity += (target - currentVelocity) * easingFactor;

      if (seqSize > 0) {
        let nextOffset = offsetRef.current + currentVelocity * deltaTime;
